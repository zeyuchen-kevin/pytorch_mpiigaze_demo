...
def _download_dlib_pretrained_model(config: yacs.config.CfgNode) -> None:
    ...
    if config.face_detector.mode != 'dlib' and config.face_detector.mode != 'mtcnn':
    ...
    dlib_model_dir = pathlib.Path('./dlib').expanduser()
...
def _download_eye_model() -> pathlib.Path:
    ...
    output_dir = pathlib.Path('./models').expanduser()
...
def _check_path(config: yacs.config.CfgNode, key: str) -> None:
    ...
    if not (path.is_file() or path.is_dir()):
        raise ValueError(f'config.{key}: {path.as_posix()} is not a file/directory.')


def _check_path_all(config: yacs.config.CfgNode) -> None:
    if config.face_detector.mode == 'dlib' or config.face_detector.mode == 'mtcnn':
...
def intersec_boxes(box1, box2):
    x1 = max(int(box1.left()), int(box2.left()))
    x2 = min(int(box1.right()), int(box2.right()))
    y1 = max(int(box1.top()), int(box2.top()))
    y2 = min(int(box1.bottom()), int(box2.bottom()))
